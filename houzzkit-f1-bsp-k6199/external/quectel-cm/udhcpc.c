...
/* Do DHCP using busybox tools */
    {
	    ...
	    //-s script
	    char *default_script="/usr/share/udhcpc/default_quectel-CM.script";
            snprintf(udhcpc_cmd, sizeof(udhcpc_cmd), "busybox udhcpc -f -n -q -t 5 -s %s -i %s", default_script, ifname);
#endif
